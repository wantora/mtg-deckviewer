...
function getCardData(str, cardData) {
  const name = removeDiacriticalMarks(str);
  const splitName = name.replace(/ *(?:\/{1,3}|&&) *.*$/, "");

  if (Object.hasOwn(cardData.cardNames, name)) {
    return cardData.cards[cardData.cardNames[name]];
  } else if (Object.hasOwn(cardData.cardNames, splitName)) {
    return cardData.cards[cardData.cardNames[splitName]];
  } else {
    return {
      name: str,
      cmc: 0,
